...
def localmax(image1,image2):
    ...
    coordinatemax=peak_local_max(thresh, min_distance=5)
    coordinatemax=corner_peaks(coordinatemax,min_distance=5)
    print(coordinatemax.shape)
    print(f'Количество локальных максимумов={coordinatemax.shape[0]}')
    for num,i in enumerate(coordinatemax.shape[0]):
        xmax.append(coordinatemax[i,1])
        ymax.append(coordinatemax[i,0])
        intensivitymax.append(imagesource[coordinatemax[i,1]],coordinatemax[i,0])
        
    d={'xmax':xmax,'ymax':ymax,'intensivitymax':intensivitymax}
    df=pd.DataFrame(data=d)
    print(df['intensivitymax'].min())
    print(df['intensivitymax'].max())
    df.to_excel("C:/Users/evgen/Downloads/localmaxintensivityfilt16parwithoutbackground.xlsx")
...
def main():
    ...
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делений
    plt.grid(True)
    localmax(imageafter8b,imageafter)
