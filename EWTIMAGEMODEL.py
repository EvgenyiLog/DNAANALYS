...
def modelimage():
    ...
    ax.grid(True)
    params = ewt_params()
    ewtc, mfb, bounds_scales, bounds_angles=ewt2dCurvelet(image, params)
    ...
    for i in range(k):
        ...
        l=np.asarray(ewtc[i])
        l=np.resize(l,(l.shape[1],l.shape[2]))
        plt.imshow(l,cmap='gray',vmax=l.max(),vmin=l.min())
        plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делений
        
        
    coeffs=pywt.swt2(imagem, 'haar',1)
    cA = coeffs 
    #cA=np.asarray(cA)
    #plt.figure(figsize=(15,7))
    ...
    ewtc, mfb, bounds_scales=ewt2dRidgelet(image, params)
    ewtc=np.asarray(ewtc)
    print(ewtc.shape)
    k=ewtc.shape[0]
    plt.figure(figsize=(15,7))
