...
def model():
    ...
    im=np.outer(signal.gaussian(512, 500), signal.gaussian(512, 500))#+np.random.random_sample((256, 256))
    for i in range(250):
    ...
    imagec=np.zeros((512,512),dtype=np.uint8)
    imagec = cv2.circle(imagec, (256,256), 250, (255,255,255), -1)
    plt.figure(figsize=(15,7))
    plt.imshow(imagec,cmap='gray',vmax=imagec.max(),vmin=imagec.min())
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делений
    
    xx, yy = np.ogrid[0:imagec.shape[0],0:imagec.shape[1]]
    fig = plt.figure(figsize=(15,7))          #create a canvas, tell matplotlib it's 3d                                                                                                              fig = plt.figure(figsize=(15,7))          #create a canvas, tell matplotlib it's 3d
    ax = fig.add_subplot(111, projection='3d')
    ax.plot_surface(xx,yy,imagec)
    ax.grid(True)
    
    imagec=cv2.filter2D(imagec, -1, im)
    plt.figure(figsize=(15,7))
    plt.imshow(imagec,cmap='gray',vmax=imagec.max(),vmin=imagec.min())
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делений
