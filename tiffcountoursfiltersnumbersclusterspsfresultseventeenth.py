...
def countourfind(image):
    ...
    df.to_csv("C:/Users/evgen/Downloads/contourap.csv")
    df.to_excel("C:/Users/evgen/Downloads/contourap.xlsx")
    d={'lightness ':l[ycentr,xcentr],'value ':v[ycentr,xcentr],'Saturation':s[ycentr,xcentr],'hue':h[ycentr,xcentr]}
    df=pd.DataFrame(data=d)
    
    
    
    df.to_csv("C:/Users/evgen/Downloads/contourlvhs.csv")
    df.to_excel("C:/Users/evgen/Downloads/contourlvhs.xlsx")
...
def lvfindcountur(edges):
    ...
    contoursl, hierarchyl= cv2.findContours(threshl, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    #print('Координаты центра')
    xcentrl=[]
    ycentrl=[]
    areasl=[]
    ...
    d={'xcentr':xcentrl,'ycentr':ycentrl,'areas': areasl,'perimeters':perimetersl}
    df=pd.DataFrame(data=d)
    
    df.to_csv("C:/Users/evgen/Downloads/contourl.csv")
    df.to_excel("C:/Users/evgen/Downloads/contourl.xlsx")  


    hsv=cv2.cvtColor(edgesrgb,cv2.COLOR_RGB2HSV)
    h,s,v=cv2.split(hsv)
    retv, threshv = cv2.threshold(v, 0, 1, 0)
    contoursv, hierarchyv = cv2.findContours(threshv, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    #print('Координаты центра')
    xcentrv=[]
    ycentrv=[]
    areasv=[]
    ...
    print(len(xcentrv))
    print()
    #d={'xcentr':xcentrv,'ycentr':ycentrv,'areas': areasv,'perimeters':perimetersv}
    #df=pd.DataFrame(data=d)
    
    #df.to_csv("C:/Users/evgen/Downloads/contourv.csv")
    #df.to_excel("C:/Users/evgen/Downloads/contourv.xlsx")  
