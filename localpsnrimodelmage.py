...
def main():
    ...
    image_max = ndi.maximum_filter(image, size=2, mode='constant')
    image_std=std_convoluted(image, 2)
    psnr=20*np.log10(np.divide(image_max,image_std))
    plt.figure(figsize=(15, 7))
    plt.imshow(psnr, cmap=plt.cm.gray,vmax=psnr.max(),vmin=psnr.min())
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делений
    plt.show()
