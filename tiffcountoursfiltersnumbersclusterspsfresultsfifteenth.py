...
def filtration(image,path):
    ...
    correlation = np.zeros_like(hlpfilt)
    d=1
    

    for i in range(d, sh_row - (d + 1)):
        for j in range(d, sh_col - (d + 1)):
            correlation[i, j] = correlation_coefficient(hlpfilt[i - d: i + d + 1,
                                                        j - d: j + d + 1],
                                                    image2[i - d: i + d + 1,
                                                        j - d: j + d + 1])
    correlation=np.asarray(correlation,dtype=np.uint8)
    r=cv2.cvtColor(corelation, cv2.COLOR_GRAY2BGR) 
    cv2.imwrite("C:/Users/evgen/Downloads/s_1_1102_c_a_normcorr.jpg",r)
    ...
    adjusted = cv2.convertScaleAbs(hlpfilt, alpha=alpha, beta=beta)
    r=cv2.cvtColor(adjusted, cv2.COLOR_GRAY2BGR) 
    cv2.imwrite("C:/Users/evgen/Downloads/s_1_1102_c_adjusted.jpg",r)
    ...
    out = cv2.addWeighted(hlpfilt, contrast, hlpfilt, 0, brightness)
    r=cv2.cvtColor(out, cv2.COLOR_GRAY2BGR) 
    cv2.imwrite("C:/Users/evgen/Downloads/s_1_1102_c_addweighted.jpg",r)
