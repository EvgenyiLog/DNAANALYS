...
def cepstrumimage(image):
    # Compute the discrete Fourier Transform of the image
    fourier = cv2.dft(np.float32(image), flags=cv2.DFT_COMPLEX_OUTPUT)
    fourier_shift = np.fft.fftshift(fourier)
    cepstrum=cv2.idft(np.log2(cv2.magnitude(fourier_shift[:,:,0],fourier_shift[:,:,1])))
    # Scale the magnitude for display
    cepstrum = cv2.normalize(cepstrum, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8UC1)
    plt.figure(figsize=(15, 7))
    plt.imshow(cepstrum[0:1000,0:1000], cmap=plt.cm.gray,vmax=cepstrum.max(),vmin=cepstrum.min())
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делений
    #print(np.amax(cepstrum))
