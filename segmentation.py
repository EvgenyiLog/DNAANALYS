...
def segmentation(images,k=3):
    q25, q75 = np.percentile(images, 25), np.percentile(images, 75)
    iqr = q75 - q25
    cut_off = iqr * k
    lower, upper = q25 - cut_off, q75 + cut_off
    
    mask=cv2.inRange(images, lower , upper)
    image=cv2.bitwise_and(images, images, mask=mask)
    print(image.dtype)
    plt.figure(figsize=(15,7))
    ...
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делений
    cv2.imwrite("C:/Users/evgen/Downloads/s_1_1102_c_seg.jpg",image)
    xx, yy = np.ogrid[0:image.shape[0],0:image.shape[1]]
    fig = plt.figure(figsize=(15,7))          #create a canvas, tell matplotlib it's 3d
    ax = fig.add_subplot(111, projection='3d')
    #ax.scatter(xs = xx, ys = yy, zs = image)
    ax.plot_surface(xx,yy,image)
    ax.grid(True)
    ret, thresh = cv2.threshold(images,0,4096,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    plt.figure(figsize=(15,7))
    plt.imshow(thresh[0:1000,0:1000],cmap='gray',vmax=thresh.max(),vmin=thresh.min())
    #plt.grid(True)
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делений
    images=cv2.normalize(images, None, 0, 256, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    thresh=cv2.adaptiveThreshold(images,256,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,3,0)
    plt.figure(figsize=(15,7))
    plt.imshow(thresh[0:1000,0:1000],cmap='gray',vmax=thresh.max(),vmin=thresh.min())
    #plt.grid(True)
    plt.tick_params(labelsize =20,#  Размер подписи
    ...
    thresh=cv2.adaptiveThreshold(images, 256,cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY,3,0)
    plt.figure(figsize=(15,7))
    plt.imshow(thresh[0:1000,0:1000],cmap='gray',vmax=thresh.max(),vmin=thresh.min())
    #plt.grid(True)
    plt.tick_params(labelsize =20,#  Размер подписи
    ...
    # Marker labelling
    ret, markers = cv2.connectedComponents(images)
    # Add one to all labels so that sure background is not 0, but 1
    markers = markers+1
    # Now, mark the region of unknown with zero
    markers[images==255] = 0
    plt.figure(figsize=(15,7))
    plt.imshow(images[0:1000,0:1000],cmap='gray',vmax=images.max(),vmin=images.min())
    #plt.grid(True)
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делений
    
    #markers = cv2.watershed(images,markers)
    #images[markers == -1] = [255,0,0]
    #plt.figure(figsize=(15,7))
    #plt.imshow(images[0:1000,0:1000],cmap='gray',vmax=images.max(),vmin=images.min())
    #plt.grid(True)
    #plt.tick_params(labelsize =20,#  Размер подписи
