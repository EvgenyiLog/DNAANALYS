...
def main():
    ...
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делений 
    ret,thresh1 = cv2.threshold(image,0,4096,cv2.THRESH_BINARY)
    ret,thresh2 = cv2.threshold(image,0,4096,cv2.THRESH_BINARY_INV)
    ret,thresh3 = cv2.threshold(image,0,4096,cv2.THRESH_TRUNC)
    ret,thresh4 = cv2.threshold(image,0,4096,cv2.THRESH_TOZERO)
    ret,thresh5 = cv2.threshold(image,0,4096,cv2.THRESH_TOZERO_INV)
    ret,thresh6 = cv2.threshold(image,0,4096,cv2.THRESH_OTSU)
    titles = ['Original Image','BINARY','BINARY_INV','TRUNC','TOZERO','TOZERO_INV','OTSU']
    images = [image, thresh1, thresh2, thresh3, thresh4, thresh5,thresh6]
    for i in range(7):
        plt.subplot(1,7,i+1),plt.imshow(images[i],'gray',vmin=0,vmax=4096)
        plt.title(titles[i])
        plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делений
    ...
    try:
        th2 = cv2.adaptiveThreshold(image,255,cv2.ADAPTIVE_THRESH_MEAN_C,\
                                    cv2.THRESH_BINARY+cv2.THRESH_OTSU,3,5)
        print('sucsessfull mean')
    except:
        th2=th1
    try:
        th3 = cv2.adaptiveThreshold(image,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,\
                                    cv2.THRESH_BINARY+cv2.THRESH_OTSU,3,5)
        print('sucsessfull gauss')
    except:
        th3=th2
    titles = ['Original Image', 'Global Thresholding (v = 255)',
    'Adaptive Mean Thresholding', 'Adaptive Gaussian Thresholding']
    images = [image, th1, th2, th3]
    for i in range(4):
        plt.subplot(2,2,i+1),plt.imshow(images[i],'gray',vmin=0,vmax=4096)
        plt.title(titles[i]) 
        plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делений
    ...
    imagewA = cv2.warpAffine(image,M,(cols,rows))
    plt.figure(figsize=(15, 7))
    plt.imshow(imagewA, cmap=plt.cm.gray,vmax=imagewA.max(),vmin=imagewA.min())
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делени
    ...
    plt.figure(figsize=(15, 7))
    plt.imshow(dst,cmap=plt.cm.gray,vmax=dst.max(),vmin=dst.min())
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делени
    ...
    dst = cv2.warpAffine(image,M,(cols,rows))
    plt.figure(figsize=(15, 7))
    plt.subplot(121),plt.imshow(image,cmap=plt.cm.gray,vmax=image.max(),vmin=image.min()),plt.title('Input')
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делени
    plt.subplot(122),plt.imshow(dst,cmap=plt.cm.gray,vmax=dst.max(),vmin=dst.min()),plt.title('Output')
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делени
