...
import pywt
from astropy.modeling.models import RickerWavelet2D
import matplotlib.pyplot as plt
from mpl_toolkits.mplot3d.axes3d import Axes3D
from matplotlib import cm
def rikcerconvolve(image):
    image=cv2.normalize(image, None, 0, 4096, cv2.NORM_MINMAX, dtype=cv2.CV_16U)
    xx, yy = np.ogrid[0:20,0:20]
    wp=np.amax(image)*(1-(np.square(np.subtract(xx,5))+np.square(np.subtract(yy,5)))/np.var(image))*np.exp(-(np.square(
        np.subtract(xx,5))+np.square(np.subtract(yy,5)))/2*np.var(image))
    print(wp.shape)
    
    fig = plt.figure(figsize=(15,7))          #create a canvas, tell matplotlib it's 3d
    ax = fig.add_subplot(111, projection='3d')
    #ax.scatter(xs = xx, ys = yy, zs = image)
    ax.plot_surface(xx,yy,wp)
    ax.grid(True)
    xx, yy = np.ogrid[0:image.shape[0],0:image.shape[1]]
    print(xx.shape)
    fig = plt.figure(figsize=(15,7))          #create a canvas, tell matplotlib it's 3d
