...
def main():
    ...
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делений
    himage=signal.hilbert2(image)
    print(image.shape)
    grad=np.divide(np.gradient(np.unwrap(np.angle(himage))),(2*np.pi))
    x_grad = grad[0]
    y_grad = grad[1]
    plt.figure(figsize=(15, 7))
    plt.subplot(131)
    ...
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делени
    plt.subplot(132)
    plt.imshow(x_grad, cmap=plt.cm.gray,vmax=x_grad.max(),vmin=x_grad.min())
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делени
    plt.subplot(133)
    plt.imshow(y_grad, cmap=plt.cm.gray,vmax=y_grad.max(),vmin=y_grad.min())
    plt.tick_params(labelsize =20,#  Размер подписи
                    color = 'k')   #  Цвет делени
    plt.show()
    plt.show()
